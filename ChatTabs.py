...
def list_lateFee_Balance():
    ...
    cur.execute("""
                ...
                ORDER BY
                    LateFee_Balance;
                """, (vCard_entry.get(), vCard_entry.get() if vCard_entry.get() != "" else None,
                      "%" + vname_entry.get() + "%", vname_entry.get() if vname_entry.get() != "" else None))
...
def list_bookInfo_view():
    ...
    cur.execute("""
                ...
                """, (vCard_entry.get(), vCard_entry.get() if vCard_entry.get() != "" else None,
                      book_id_entry.get(), "%" + book_title_entry.get() + "%", book_title_entry.get() if book_title_entry.get() != "" else None))

    vBookInfo_text = cur.fetchall()
    conn.close()
    vBookInfo_text.set('\n'.join(str(book) for book in vBookInfo_text))
